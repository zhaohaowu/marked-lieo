...
// 20220224:将手写前端改为ceres自动求导
// OPTIMIZATION_TYPE: 0 高斯牛顿 1 ceres自动求导 2 ceres解析求导
#define OPTIMIZATION_TYPE 0
/**
 * 6D位姿点云结构定义
*/
...
public:
    ...
    */
    mapOptimization()
    {
        ...
        // 发布激光里程计，rviz中表现为坐标轴
        pubLaserOdometryGlobal      = nh.advertise<nav_msgs::Odometry> ("lieo/mapping/odometry", 1);
        ...
        // 订阅gt里程计
        subGT   = nh.subscribe<nav_msgs::Odometry> (gtTopic, 200, &mapOptimization::gtHandler, this, ros::TransportHints().tcpNoDelay());
    ...
    */
    void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr& msgIn)
    {
        ...
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval)//0.15s
        {
            ...
            else if(OPTIMIZATION_TYPE == 1)
                scan2MapOptimization_auto();
            else if(OPTIMIZATION_TYPE == 2)
                scan2MapOptimization_analysis();
            std::chrono::duration<double,std::ratio<1,1000>> duration_ms(std::chrono::high_resolution_clock::now()-start_time);
            static double time_sum = 0;
            time_sum += duration_ms.count();
            static int time_sum_cnt = 0;
            time_sum_cnt++;
            // std::cout << "time_sum_cnt: " << time_sum_cnt << std::endl;
            if (time_sum_cnt == 1400) {
                std::cout << "average time: " << time_sum / time_sum_cnt << std::endl;
            }
            // 设置当前帧为关键帧并执行因子图优化
            // 1、计算当前帧与前一帧位姿变换，如果变化太小，不设为关键帧，反之设为关键帧
            // 2、添加激光里程计因子、GPS因子、闭环因子
    ...
    */
    void updateInitialGuess()
    {
        ...
        if (cloudKeyPoses3D->points.empty())
        {
            ...
            if (!useImuHeadingInitialization)//这里是true，与gps有关
        ...
        static bool lastImuPreTransAvailable = false;//static变量定义只初始化一次https://blog.csdn.net/qq_16334327/article/details/78201434
        static Eigen::Affine3f lastImuPreTransformation;
        if (cloudInfo.odomAvailable == true)
        {
            // 当前帧的初始估计位姿（来自imu里程计），后面用来计算增量位姿变换
            //关于imu里程计无法提供当前帧位姿，解释为imu里程计的获得是上一帧的gtsam节点估计+到当前时刻的预积分
        ...
        // 只在第一帧调用（注意上面的return），用imu数据初始化当前帧位姿，仅初始化旋转部分，imuAvailable从第一帧就为true
    ...
    */
    void extractCloud(pcl::PointCloud<PointType>::Ptr cloudToExtract)
    {
        ...
        for (int i = 0; i < (int)cloudToExtract->size(); ++i)
        {
            ...
            //???这里没看懂，thisKeyInd来自关键帧的k紧邻半径搜索，cornerCloudKeyFrames来自后面的关键帧角点集合
        ...
        for (int i = 0; i < laserCloudCornerLastDSNum; i++)
        {
            ...
            if (pointSearchSqDis[4] < 1.0) {
                ...
                if (matD1.at<float>(0, 0) > 3 * matD1.at<float>(0, 1)) {
                    ...
                    // 根据叉积公式uxv = u1v2-u2v1, u1v3-u3v1, u2v3-u3v2
...
// 迭代30次
            ...
            // 得到优化后的transformTobeMapped[0-5]
            // std::cout << "trans: roll: " << transformTobeMapped[0] << " pitch: " << transformTobeMapped[1] << " yaw: " << transformTobeMapped[2] << " x: " << transformTobeMapped[3] << " y: " << transformTobeMapped[4] << " z: " << transformTobeMapped[5] << std::endl;
            transformUpdate();
         
        chrono::steady_clock::time_point t2 = chrono::steady_clock::now();
        static int i = 0;
        static double time_cost;
        chrono::duration<double> time_used = chrono::duration_cast<chrono::duration<double>>(t2-t1);
        time_cost += time_used.count(); 
        i++ ;
        // cout << "solve time cost = "  << "  "<< time_cost/i << "seconds" << endl;
        }
        else {
    ...
    void scan2MapOptimization_auto()
    {
        // 要求有关键帧
        if (cloudKeyPoses3D->points.empty())
            return;
...
// 迭代30次
            ...
            for (int iterCount = 0; iterCount < 2; ++iterCount)
            {
                ...
                // 优化变量:7参数 四元数+平移
                problem.AddParameterBlock(ceres_parameters, 4, q_parameterization);
                problem.AddParameterBlock(ceres_parameters + 4, 3);
                // 利用transformTobeMapped获得初始值
                updatePointAssociateToMap();
                ...
                for (int i = 0; i < laserCloudCornerLastDSNum; ++i)
                {
                    ...
                    if (pointSearchSqDis[4] < 1.0)
                    {
                        ...
                        if (saes.eigenvalues()[2] > 3 * saes.eigenvalues()[1])
                        {
                            ...
                            problem.AddResidualBlock(cost_function, loss_function, ceres_parameters, ceres_parameters + 4);
                        }
                    }
                }
                ...
                // 将ceres输出的ceres_parameters, 赋值给transformTobeMapped
                tf::Quaternion quat(ceres_parameters[0], ceres_parameters[1], ceres_parameters[2], ceres_parameters[3]);
                // std::cout << "ceres param: quat: " << ceres_parameters[0] << " , " << ceres_parameters[1] << " , " << ceres_parameters[2] << " , "<< ceres_parameters[3] << std::endl;
                // std::cout << "ceres param: dist: " << ceres_parameters[4] << " , " << ceres_parameters[5] << " , " << ceres_parameters[6] << std::endl;
                double roll = 0.0, pitch = 0.0, yaw = 0.0;       
                tf::Matrix3x3(quat).getRPY(roll, pitch, yaw);
                // std::cout << "euler: roll: " << roll << " pit: " << pitch << " yaw: " << yaw << std::endl;
                transformTobeMapped[0] = roll;
                transformTobeMapped[1] = pitch;
                transformTobeMapped[2] = yaw;
                transformTobeMapped[3] = ceres_parameters[4];// x
                transformTobeMapped[4] = ceres_parameters[5];// y
                transformTobeMapped[5] = ceres_parameters[6];// z
            }
            // 得到优化后的transformTobeMapped[0-5]
            // std::cout << "trans: roll: " << transformTobeMapped[0] << " pitch: " << transformTobeMapped[1] << " yaw: " << transformTobeMapped[2] << " x: " << transformTobeMapped[3] << " y: " << transformTobeMapped[4] << " z: " << transformTobeMapped[5] << std::endl;
            transformUpdate();
    ...
    void scan2MapOptimization_analysis()
    {
        // 要求有关键帧
        if (cloudKeyPoses3D->points.empty())
            return;
            ...
            for (int iterCount = 0; iterCount < 2; ++iterCount)
            {
                ...
                for (int i = 0; i < laserCloudCornerLastDSNum; ++i)
                {
                    ...
                    if (pointSearchSqDis[4] < 1.0)
                    {
                        ...
                        if (saes.eigenvalues()[2] > 3 * saes.eigenvalues()[1])
                        {
                            ...
                            point_b = -0.1 * unit_direction + point_on_line;
                            // 解析求导
                            ceres::CostFunction *cost_function = new LidarEdgeAnalyticCostFunction(curr_point, point_a, point_b, 1);  
                            problem.AddResidualBlock(cost_function, loss_function, ceres_parameters, ceres_parameters+4);
                ...
                for (int i = 0; i < laserCloudSurfLastDSNum; i++)
                {
                    ...
                    if (pointSearchSqDis[4] < 1.0)
                    {
                        ...
                        Eigen::Vector3d curr_point(pointOri.x, pointOri.y, pointOri.z);
                        Eigen::Vector3d last_point_a(laserCloudSurfFromMap->points[pointSearchInd[2]].x,
                                                     laserCloudSurfFromMap->points[pointSearchInd[2]].y,
                                                     laserCloudSurfFromMap->points[pointSearchInd[2]].z);
                        Eigen::Vector3d last_point_b(laserCloudSurfFromMap->points[pointSearchInd[3]].x,
                                                     laserCloudSurfFromMap->points[pointSearchInd[3]].y,
                                                     laserCloudSurfFromMap->points[pointSearchInd[3]].z);
                        Eigen::Vector3d last_point_c(laserCloudSurfFromMap->points[pointSearchInd[4]].x,
                                                     laserCloudSurfFromMap->points[pointSearchInd[4]].y,
                                                     laserCloudSurfFromMap->points[pointSearchInd[4]].z);
                        if (planeValid)
                        {
                            // 解析求导
                            ceres::CostFunction *cost_function = new LidarPlaneAnalyticCostFunction(curr_point, last_point_a, last_point_b, last_point_c, 1);    
                            problem.AddResidualBlock(cost_function, loss_function, ceres_parameters, ceres_parameters+4);
                ...
                ceres::Solver::Options options;
                options.minimizer_type = ceres::TRUST_REGION;// TRUST_REGION/LINE_SEARCH
                options.linear_solver_type = ceres::DENSE_QR;
    ...
    */
    void addOdomFactor()
    {
        if (cloudKeyPoses3D->points.empty())
        {
            ...
            // odometryNoise = noiseModel::Diagonal::Variances((Vector(6) << 1e6, 1e6, 1e6, 1e4, 1e4, 1e4).finished());
    ...
    void addGPSFactor()
    {
        ...
        while (!gpsQueue.empty())
        {
            ...
            else
            {
                ...
                // GPS噪声协方差太大，不能用
                //nav_msgs::Odometry类型数据的协方差0 7 14分别表示xyz的误差噪声
    ...
    */
    void addGTFactor()
    {
        ...
        // // 位姿协方差很小，没必要加入GT数据进行校正
        // if (poseCovariance(3,3) < poseCovThreshold && poseCovariance(4,4) < poseCovThreshold)
        //     return;

        static PointType lastGTPoint;

        while (!gtQueue.empty())
        {
            // 删除当前帧0.2s之前的里程计
            if (gtQueue.front().header.stamp.toSec() < timeLaserInfoCur - 0.2)
            {
                gtQueue.pop_front();
            }
            // 超过当前帧0.2s之后，退出
            else if (gtQueue.front().header.stamp.toSec() > timeLaserInfoCur + 0.2)
            {
                break;
            }
            else
            {
                ...
                // float noise_x = thisGT.pose.covariance[0];
                // float noise_y = thisGT.pose.covariance[7];
                // float noise_z = thisGT.pose.covariance[14];
                // if (noise_x > 2 || noise_y > 2)
                //     continue;
                float noise_x = 0.01;
                float noise_y = 0.01;
                float noise_z = 0.01;
                ...
                // (0,0,0)无效数据
                // if (abs(gt_x) < 1e-6 && abs(gt_y) < 1e-6)
                //     continue;

                // // 每隔5m添加一个GT里程计
                PointType curGTPoint;
                //xiugai
                // curGTPoint.x = gt_x;
                // curGTPoint.y = gt_y;
                // curGTPoint.z = gt_z;
                // if (pointDistance(curGTPoint, lastGTPoint) < 5.0)
                //     continue;
                // else
                //     lastGTPoint = curGTPoint;

                // 添加GT因子
                gtsam::Vector Vector3(3);
                // Vector3 << max(noise_x, 1.0f), max(noise_y, 1.0f), max(noise_z, 1.0f);
                Vector3 << noise_x, noise_y, noise_z;
                noiseModel::Diagonal::shared_ptr gt_noise = noiseModel::Diagonal::Variances(Vector3);
                gtsam::GPSFactor gt_factor(cloudKeyPoses3D->size(), gtsam::Point3(gt_x, gt_y, gt_z), gt_noise);
                gtSAMgraph.add(gt_factor);
    ...
    */
    void publishOdometry()
    {
        ...
        if (lastIncreOdomPubFlag == false)
        {
            ...
            // ofstream tum;
            // tum.open("/home/zhw/tum.txt",ios::app); //ios::trunc表示在打开文件前将文件清空,由于是写入,文件不存在则创建
            // tum  << timeLaserInfoStamp << " " <<x <<" " <<y <<" " <<z <<" " 
    ...
    */
    ...
    void savetxt()
    {
        ros::Rate rate(0.2);
        while (ros::ok()){
            if (cloudKeyPoses6D->points.empty() == true)
                continue;
            rate.sleep();
            ofstream in;
            in.open("/home/zhw/com.txt",ios::trunc); //ios::trunc表示在打开文件前将文件清空,由于是写入,文件不存在则创建
            for(int i=0; i<cloudKeyPoses6D->points.size(); i++)
            {
                in<<cloudKeyPoses6D->points[i].x;
                in<<" ";
            }
            cout<<"1111111"<<endl;
            in.close();//关闭文件
...
int main(int argc, char** argv)
{
    ...
    // std::thread savetxt(&mapOptimization::savetxt, &MO);
    //这两个线程同时运行，直到被join或者detach
    ros::spin();
